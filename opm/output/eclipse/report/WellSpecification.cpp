...
namespace {
    ...
    /// contents of those well lists that have changed since the previous
    /// report.
    ///
    ///          WELL LISTS (Date)
    ///          -----------------
    ...
    ///
    /// \param[in] changedWellLists Well lists whose contents have changed
    ...
    ///
    /// \param[in,out] Stream to which to write the well list report.
    void emitWellLists([[maybe_unused]] const std::vector<std::string>& changedWellLists,
                       const Opm::Schedule&            schedule,
                       const std::size_t               reportStep,
                       std::ostream&                   os)
    {
        const auto indent = std::string(std::string::size_type{10}, ' ');
        ...
        //
        {
            const auto timepoint = [simTime = schedule.simTime(reportStep)]() mutable
            {
                return *std::localtime(&simTime);
            }();

            const auto header = ::Opm::uppercase
                (fmt::format("WELL LISTS ({:%d-%b-%Y})", timepoint));

            os << "\n\n"
               << indent << std::string(std::string::size_type{40}, ' ')
               << header << '\n'
               << indent << std::string(std::string::size_type{40}, ' ')
               << std::string(header.size(), '-') << "\n\n";
        }

        const auto hline = std::string(std::string::size_type{113}, '-');
                                              ...
                                              std::ostream&                   os)
{
    ...
    if (! changedWellLists.empty()) {
        emitWellLists(changedWellLists, schedule, reportStep, os);
    }
