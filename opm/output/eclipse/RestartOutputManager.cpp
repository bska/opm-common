...
Opm::RestartIO::RestartOutputManager&
Opm::RestartIO::RestartOutputManager::limitRestartOutput(const int norst)
{
    if (norst > 0) {
        this->permitExtendedArrays_ = false;
    }

    if (norst <= 0) {
        this->fileCategory_ = FileCategory::FullRestart;
    }
    else if (norst == 1) {
        this->fileCategory_ = FileCategory::GraphicsOnly;
    }
    else {
        this->fileCategory_ = FileCategory::NoWell;
    }
                 ...
                 EclIO::OutputStream::RestartBase& rstFile) const
{
                          ...
                          [isFip = IsFluidInPlace{}, this](const auto& prtIdx)
    {
        ...
        if (this->permitExtendedArrays_ && (this->writeContext_->gridIndex == 0)) {
            // Extra solution arrays (OPMEXTRA &c) are only supported for the main grid.
            // Note that these are intentionally emitted *after* ENDSOL...
            this->extraSolutionArrays();
        }
...
Opm::RestartIO::RestartOutputManager::prologue() const
{
    ...
    if (this->fileCategory_ == FileCategory::FullRestart) {
...
{
    if (this->fileCategory_ != FileCategory::FullRestart) {
        return; // Group data is only relevant for full restarts.
    }

    const auto simStep = static_cast<std::size_t>
        (std::max(this->writeContext_->reportStep - 1, 0));

    auto groupData = Helpers::AggregateGroupData {
        this->writeContext_->inteHead
    };
...
Opm::RestartIO::RestartOutputManager::networks() const
{
    ...
    auto networkData = Helpers::AggregateNetworkData {
        this->writeContext_->inteHead
    };
...
{
    if (this->fileCategory_ != FileCategory::FullRestart) {
        return; // Aquifer data is only relevant for full restarts.
    }
...
{
    if (this->fileCategory_ != FileCategory::FullRestart) {
        return; // ACTIONX data is only relevant for full restarts.
    }
...
{
    if (this->fileCategory_ != FileCategory::FullRestart) {
        return; // Special solution arrays are only relevant for full restarts.
    }
...
Opm::RestartIO::RestartOutputManager::solutionNNC() const
{
    ...
    auto nncArrays = allArrays |
    std::views::filter([nncRE = std::regex{ R"(FL[OR](OIL|GAS|WAT)N\+)" }](const auto& array)
    {
        return std::regex_match(array.first.key, nncRE);
    }) |
    std::views::transform([](const auto& array)
    {
        // Transform to a pair of array name and CellData for easier handling downstream.
        const auto& [key, value] = array;

        return std::pair<std::string, data::CellData> {
            std::piecewise_construct,
            std::forward_as_tuple(key.key),
            std::forward_as_tuple(key.dim, value, data::TargetType::RESTART_SOLUTION)
        };
    });

    this->writeSolutionValues(nncArrays);
}
...
{
    if (this->fileCategory_ != FileCategory::FullRestart) {
        return; // Multisegment well arrays are only relevant for full restarts.
    }
...
Opm::RestartIO::RestartOutputManager::writeWellArrays() const
{
    ...
    // Note: We emit IWEL and ZWEL for all file categories.
    //
    // While arguably against the spirit of NORST, these are so useful
    // for visualization and flow diagnostics in post-processing tools
    // like ResInsight that it's worth the extra cost of writing them
    // even for the GraphicsOnly and NoWell file types.
    ...
    if (this->fileCategory_ != FileCategory::NoWell) {
...
{
    if (this->fileCategory_ != FileCategory::FullRestart) {
        return; // Well list arrays pertain to full restarts only.
    }
...
{
    if (this->fileCategory_ == FileCategory::NoWell) {
        return; // No well connection data to write for this file category.
    }
    ...
    // Emit XCON for both FullRestart and GraphicsOnly since it's
    // useful for flow-diagnostics analysis in post-processing tools
    // like ResInsight.
...
Opm::RestartIO::RestartOutputManager::regularSolutionArrays(const IsFluidInPlace& isFip) const
{
    ...
    this->writeSolutionValues(regularArrays);
}
...
Opm::RestartIO::RestartOutputManager::fluidInPlaceArrays(const IsFluidInPlace& isFip) const
{
    ...
    this->writeSolutionValues(fipArrays);
    ...
    this->writeSolutionValues(fipArrays | std::views::transform([](const auto& array)
    {
        return std::make_pair('S' + array.first, array.second);
    }));
}
...
Opm::RestartIO::RestartOutputManager::tracerArrays() const
{
        ...
        -> std::pair<std::string_view, std::vector<EclIO::PaddedOutputString<8>>>
    {
        return {
            ...
            std::forward_as_tuple(std::initializer_list {
        ...
        -> std::pair<std::string_view, std::vector<EclIO::PaddedOutputString<8>>>
    {
        ...
        auto lookupName = tracerName;
        lookupName.remove_suffix(1);
        const auto& tracerUnit = trConfig[std::string { lookupName }].unit_string;
    ...
    // Note: We can't use writeSolutionValues() here since we need to write
    // the tracer header before each tracer array, which requires interleaving
    // header writing with array writing.  Instead, we handle the tracer
    // arrays separately here to allow for this interleaving.
...
Opm::RestartIO::RestartOutputManager::thresholdPressureArray() const
{
    ...
    if (this->solutionArrayBackend_.has_value()) {
        this->writeSolutionValues(thpressArrays | std::views::transform([](const auto& array)
            -> std::pair<std::string, data::CellData>
        {
            const auto& [key, value] = array;
        ...
        // Note: Can't use writeSolutionValues() here since we need to output the
        // threshold pressure array as double precision regardless of the original
        // precision in the solution state.
        //
        // That also means performing manual unit conversion and writing the array
        // using writeArray() instead of delegating to defaultSolutionArrayBackend().
...
Opm::RestartIO::RestartOutputManager::extendedSolutionArrays() const
{
    ...
    this->writeSolutionValues(extendedArrays);
}

void
Opm::RestartIO::RestartOutputManager::extraSolutionArrays() const
{
    const auto& allArrays = this->writeContext_->state.get()
        .solution[this->writeContext_->gridIndex].extra;

    auto extraArrays = allArrays |
        std::views::filter([nncRE = std::regex{ R"(FL[OR](OIL|GAS|WAT)N\+)" }](const auto& array)
    {
        return (array.first.key != "THRESHPR")
            && !std::regex_match(array.first.key, nncRE);
    }) |
    std::views::transform([](const auto& array)
    {
        // Transform to a pair of array name and CellData for easier handling downstream.
    ...
    this->writeSolutionValues(extraArrays);
}
...
void
Opm::RestartIO::RestartOutputManager::
defaultSolutionArrayBackend(const std::string&    arrayName,
