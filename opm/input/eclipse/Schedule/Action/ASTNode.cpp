...
Opm::Action::Result
Opm::Action::evalComparison(const Context& context) const
{
    auto v2 = Value {};

    // Special casing of MONTH comparisons where in addition to symbolic
    // month names we can compare with numeric months.  When conducting
    // numeric months comparisons, the numeric value should be rounded
...
std::vector<std::string>
Opm::Action::ASTNode::getWellList(Context& contexct) const
{
