...
namespace {
    ...
    struct OutputControls
    {
        ...
        std::optional<int> norst{};
    };

    std::tuple<std::map<std::string, int, std::less<>>, OutputControls>
           ...
           const bool               compositional = false)
    {
        ...
        auto ctrls = OutputControls{
            .basic = extract("BASIC", mnemonics),
            .freq  = extract("FREQ" , mnemonics),
            .norst = extract("NORST", mnemonics)
        };

        expand_RPTRST_mnemonics(mnemonics);

        return { mnemonics, ctrls };
    }
...
void RSTConfig::handleRPTRST(const DeckKeyword&  keyword,
                             ...
                             ErrorGuard&         errors)
{
    const auto& [mnemonics, ctrls] =
        RPTRST(keyword, parseContext, errors, this->compositional_);

    this->update_schedule(ctrls.basic, ctrls.freq);
    update_optional(ctrls.norst, this->norst_);
                                     ...
                                     ErrorGuard&         errors)
{
    ...
    update_optional(ctrls.basic, this->basic_);
    update_optional(ctrls.freq , this->freq_ );
    update_optional(ctrls.norst, this->norst_);
