...
NameOrder::sort(const std::vector<std::string>& names) const
{
    auto sortIx = std::vector<std::size_t>{};
    sortIx.reserve(names.size());
    std::transform(names.begin(), names.end(), std::back_inserter(sortIx),
                   [this](const auto& name) { return this->m_index_map.at(name); });

    auto nameIx = std::vector<std::vector<std::size_t>::size_type>(sortIx.size());
    std::iota(nameIx.begin(), nameIx.end(), std::vector<std::size_t>::size_type{});

    std::sort(nameIx.begin(), nameIx.end(),
              [&sortIx](const auto& i1, const auto& i2)
              { return sortIx[i1] < sortIx[i2]; });

    auto sortedNames = std::vector<std::string>{};
    sortedNames.reserve(names.size());
    for (const auto& i : nameIx) {
