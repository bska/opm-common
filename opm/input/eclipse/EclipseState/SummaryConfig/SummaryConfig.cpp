...
namespace {
    ...
    {
        /// Region set -> distinct region numbers (e.g., "FIPABC" -> { 1, 2, 4, 7 })
        std::unordered_map<std::string, std::set<int>> regions{};

        /// Vector -> Well -> Connections/Cartesian cell indices (e.g,
        /// "COPR" -> "P-1" -> { 12, 34, 56, 78, 910 })
        std::unordered_map<std::string, std::unordered_map<std::string, std::unordered_set<std::size_t>>> uniqueConnVectors{};
    };
...
void handleConnectionCell(const bool                       isGeomechRun,
                          ...
                          std::unordered_set<std::size_t>& knownConns,
                          SummaryConfig::keyword_list&     list,
                          SummaryConfig::keyword_list&     extraFracturingVectors)
{
    ...
    if (isGeomechRun) {
        for (auto i = 0*NumExtraAllocatedGeomechConns; i < NumExtraAllocatedGeomechConns; ++i) {
            extraFracturingVectors.push_back(param.number(-1));
        }
    }
}
...
void connKeywordDefaultedConns(const bool                                                        isGeomechRun,
                               ...
                               std::unordered_map<std::string, std::unordered_set<std::size_t>>& uniqueVectors,
                               SummaryConfig::keyword_list&                                      list,
                               SummaryConfig::keyword_list&                                      extraFracturingVectors)
{
    auto param = param0;
    ...
    for (const auto& wellName : wellNames) {
        ...
        auto& knownConns = uniqueVectors[wellName];
                       ...
                       SummaryConfig::keyword_list& extraFracturingVectors)
{
    ...
    auto& uniqueVectors = context.uniqueConnVectors[keyword.name()];
...
{
    try {
        const SUMMARYSection section(deck);
        SummaryConfigContext context;
